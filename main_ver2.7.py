...
# Show informative message
                        silent_side = "izquierdo" if left_silent else "derecho"
                        ...
                        if left_silent and not right_silent:
                            mono_data = right_channel  # Usar canal derecho si izquierdo está en silencio
                        elif right_silent and not left_silent:
                            mono_data = left_channel  # Usar canal izquierdo si derecho está en silencio
                        else:
                            mono_data = left_channel  # Por defecto usar canal izquierdo
                    else:
                        mono_data = left_channel  # Comportamiento original si ambos canales tienen audio
                else:
    ...
    for root_dir, _, files in os.walk(folder):
        ...
        for file in wav_files:
            ...
            try:
                ...
                if audio_type == "mono_convert":
                    new_name = f"{os.path.splitext(file)[0]} (mono).wav"
                    data, sample_rate = sf.read(original_path)
                    left_channel = data[:, 0]
                    right_channel = data[:, 1]
                    
                    if np.all(np.abs(left_channel) < 1e-10):
                        mono_data = right_channel
                    else:
                        mono_data = left_channel
                    
                    sf.write(os.path.join(root_dir, new_name), mono_data, sample_rate)
                    print(f"File converted to mono: {original_path} -> {new_name}")
        ...
        if len(data.shape) == 1 or data.shape[1] == 1:
            ...
            if left_silent or right_silent:
                ...
                messagebox.showinfo("Silent Channel Detected", 
                    ...
                    "It will be automatically converted to mono using the active channel.")
                return "mono_convert"  # Nuevo tipo para indicar conversión automática
...
def convert_to_stereo(left_file_path, right_file_path, progress_window=None):
    ...
    if progress_window:
        progress_window.add_log(f"Saving stereo file: {os.path.basename(output_file)}")
